...
wechatIo.on('connection', (socket) => {
    socket.on('crawler', (crawData) => {
        ...
        /*
        fs.writeFile(path.join(output, `${newData.otitle}.html`, content,
         function (err) {
            if (err) {
...
module.exports = {
    ...
    * beforeSendResponse(requestDetail, responseDetail) {
        // 历史文章列表
        if (requestDetail.url.indexOf('mp.weixin.qq.com/mp/profile_ext?') !==
            -1 && requestDetail.requestOptions.method === 'GET') {
            ...
            requestDetail.url.indexOf('mp.weixin.qq.com/mp/getappmsgext?') !==
            -1 &&
